...
<body>
 <div class="mianCont">
  ...
  <div class="lsg">
   ...
   <div class="gouwuche">
    <div class="gouCar">
     ...
     <a href="/st-web/jsp/order.jsp">去结算</a> <img src="../images/youjian.jpg" width="5" height="8" />    
    </div><!--gouCar/-->
    <div class="myChunlv">
     <a href="/st-web/jsp/vip.jsp"><img src="../images/mychunlv.jpg" width="112" height="34" /></a>
    </div><!--myChunlv/-->
   </div><!--gouwuche/-->
  </div><!--lsg/-->
  <div class="pnt">
  ...
  <form action="../dingdanAction/addDingdan.do"  method="post">
  <div class="positions">
   当前位置：<a href="/st-web/jsp/index.jsp">首页</a> &gt; <a href="#" class="posCur">填写核对订单</a>
  </div><!--positions/-->
  <div class="cont">
   ...
   <table class="vipAdress">
    ...
    <% List<GouwucheModel> address = (List<GouwucheModel>)SessionUtils.getAttr(request, "chadizhi"); %>
    <% for(int i = 0; i < address.size(); i++){
    	GouwucheModel address1 = address.get(i);
     %>
    
    <tr>
     <td width="30%">
     <input type="hidden" value="<%=address1.getId()%>" name="sh_dizhi_id"/>
      <input type="radio" name="name"/><%=address1.getName() %></td>
     <td width="50%"><%=address1.getXxaddress() %></td>
     <td><%=address1.getTelephone()%></td>
    </tr>
    <% } %>
   </table><!--ord/-->
  ...
  <table class="orderList">
    ...
    <% List<GouwucheModel> list = (List)SessionUtils.getAttr(request, "listdindgan");%>
    <% for(int i = 0; i < list.size(); i++ ){
    	GouwucheModel gouwuche = list.get(i);
     %>
    <tr>
     <td><input type="checkbox" id="checkbox<%=gouwuche.getId() %>" value="<%=gouwuche.getId()%>" name="sp_id" /></td>
     <td>
      <dl>
       <dt><a href="/st-web/jsp/proinfo.jsp"><img src="<%=gouwuche.getUrl()%>" width="85" height="85" /></a></dt>
       <dd><%=gouwuche.getName() %><br /><span class="red">有货：</span>从上海出发</dd>
       <div class="clears"></div>
      </dl>
     </td>
     <td>￥<strong class="red" id="danjia<%=gouwuche.getId()%>" ><%= gouwuche.getPrice() %></strong></td>
     <td>
     <div class="jia_jian">
      <img src="../images/jian.jpg" width="21" height="25" class="jian" onclick="jian('<%=gouwuche.getId()%>')"/>
      <input type="text" class="shuliang" name="quantity" value="<%= gouwuche.getQuantity() %>" id="shuliang<%=gouwuche.getId() %>"/>
      <img src="../images/jia.jpg" width="21" height="25" class="jia" onclick="jia('<%=gouwuche.getId()%>')"/>
     </div>
     </td>
     
     <td>￥<strong class="red"  id="zongjine<%=gouwuche.getId()%>"  name="money"><%=gouwuche.getPrice() %></strong></td>
    </tr>
     <%} %> 
   </table><!--orderList/-->
   <table class="zongjia" align="right">
   <tr>
     <td>合计：<strong class="red">+2271.88</strong></td>
    </tr>
    <tr>
     <td colspan="2" style="height:50px;">
     <input type="image" src="../images/tijao.png"  width="142" height="32"/>
     </td>
    </tr>
   </table><!--zongjia/-->
